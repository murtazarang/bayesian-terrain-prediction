...
class EncoderDecoderWrapper3d(nn.Module):
    def __init__(self, args, encoder, decoder_cell, feature_list, n_features=None, decoder_input=True):
        ...
        if self.args.model in ['3d', '3D']:
            ...
            self.convlstm2d_dec = nn.ModuleList([
                ...
                ConvLSTMCell(self.args, self.c2, 1, kernel_size=(1, 1), bias=True)
            ])
            ...
            self.conv3d_dec = nn.Conv3d(self.args.preenc_t, 1, kernel_size=(1, 3, 3), padding=(0, 1, 1))
    ...
    def forward(self, xb):
        ...
        if self.args.model in ['1d', '1D']:
            x_enc_in = torch.cat((x_enc[0], x_enc[-4], x_enc[-3], x_enc[-2]), -1)
            x_dec_in = torch.cat((x_dec[-4], x_dec[-3], x_dec[-2]), -1)
        ...
        # Convert to Time Major for 3D
            ...
            if self.args.use_add_features:
                x_lin_enc = torch.cat([x_enc[-4], x_enc[-3], x_enc[-2]], dim=-1)
                x_lin_dec = torch.cat([x_dec[-4], x_dec[-3], x_dec[-2]], dim=-1)
